#!/bin/bash
# build_rpm.sh -- Build and sign RPMs via CPack
set -euo pipefail
...
echo "Building RPM packages for abls-agent-libs..."
...
rm -f "$BUILD_DIR"/abls-agent-libs-*.rpm "$BUILD_DIR"/abls-agent-libs-devel-*.rpm

pushd "$BUILD_DIR" >/dev/null
cpack -G RPM
popd >/dev/null

runtime_rpm=$(find "$BUILD_DIR" -maxdepth 1 -type f -name 'abls-agent-libs-[0-9]*.rpm' | sort | tail -n 1)
devel_rpm=$(find "$BUILD_DIR" -maxdepth 1 -type f -name 'abls-agent-libs-devel-[0-9]*.rpm' | sort | tail -n 1)
