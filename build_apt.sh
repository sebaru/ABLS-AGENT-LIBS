#!/bin/bash
# build_apt.sh -- Build DEB packages for abls-agent-libs via CPack
set -euo pipefail
...
echo "Building DEB packages for abls-agent-libs..."
...
if [[ "$CLEAN" == "true" ]]; then
  rm -f "$BUILD_DIR"/abls-agent-libs*.deb
  rm -f "$ARTIFACT_DIR"/abls-agent-libs*.deb
fi
...
while IFS= read -r deb_file; do
  if command -v dpkg-deb >/dev/null 2>&1; then
    ...
    if [[ "$pkg_name" == "abls-agent-libs" ]]; then
      runtime_deb="$deb_file"
      continue
    fi
    if [[ "$pkg_name" == "abls-agent-libs-dev" || "$pkg_name" == "abls-agent-libs-devel" ]]; then
      devel_deb="$deb_file"
      continue
    fi
  fi
